...
type Props = {
  ticket_D_isDownload: boolean;
  setTicket_D_isDownload: (value: boolean) => void;
};

const TicketD = ({ ticket_D_isDownload, setTicket_D_isDownload }: Props) => {
  const EVENT_TYPE = "D";
  ...
  const ticket_D_ref = useRef<HTMLCanvasElement | null>(null);
  useEffect(() => {
    // ANCHOR : TICKET
    const ticket_canvas = ticket_D_ref.current;
    if (ticket_canvas) {
  ...
  useEffect(() => {
    if (ticket_D_isDownload) {
      downloadTicketD();
      setTicket_D_isDownload(false);
    }
  }, [ticket_D_isDownload]);

  const downloadTicketD = () => {
    const ticket_canvas = ticket_D_ref.current;
    if (ticket_canvas) {
      const link = document.createElement("a");
      link.href = ticket_canvas.toDataURL("image/png");
      link.download = "18JUNE-e-Ticket.png";
      link.click();
    }
  };
