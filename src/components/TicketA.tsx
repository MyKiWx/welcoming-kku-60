...
type Props = {
  ticket_A_isDownload: boolean;
  setTicket_A_isDownload: (value: boolean) => void;
};

const TicketA = ({ ticket_A_isDownload, setTicket_A_isDownload }: Props) => {
  const EVENT_TYPE = "A";
  ...
  const ticket_A_ref = useRef<HTMLCanvasElement | null>(null);
  useEffect(() => {
    // ANCHOR : TICKET
    const ticket_canvas = ticket_A_ref.current;
    if (ticket_canvas) {
  ...
  useEffect(() => {
    if (ticket_A_isDownload) {
      downloadTicketA();
      setTicket_A_isDownload(false);
    }
  }, [ticket_A_isDownload]);

  const downloadTicketA = () => {
    const ticket_canvas = ticket_A_ref.current;
    if (ticket_canvas) {
      const link = document.createElement("a");
      link.href = ticket_canvas.toDataURL("image/png");
      link.download = "12JUNE-e-Ticket.png";
      link.click();
    }
  };
