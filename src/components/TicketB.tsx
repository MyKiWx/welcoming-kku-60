...
type Props = {
  ticket_B_isDownload: boolean;
  setTicket_B_isDownload: (value: boolean) => void;
};

const TicketB = ({ ticket_B_isDownload, setTicket_B_isDownload }: Props) => {
  const EVENT_TYPE = "B";
  ...
  const ticket_B_ref = useRef<HTMLCanvasElement | null>(null);
  useEffect(() => {
    // ANCHOR : TICKET
    const ticket_canvas = ticket_B_ref.current;
    if (ticket_canvas) {
  ...
  useEffect(() => {
    if (ticket_B_isDownload) {
      downloadTicketB();
      setTicket_B_isDownload(false);
    }
  }, [ticket_B_isDownload]);

  const downloadTicketB = () => {
    const ticket_canvas = ticket_B_ref.current;
    if (ticket_canvas) {
      const link = document.createElement("a");
      link.href = ticket_canvas.toDataURL("image/png");
      link.download = "13JUNE-e-Ticket.png";
      link.click();
    }
  };
